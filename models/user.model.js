        ...
        validate: {
            isValidProviders(value) {
                if (value && Array.isArray(value)) {
                    ...
                    const invalidProviders = value.filter(provider => !validProviders.includes(provider));
                    if (invalidProviders.length > 0) {
                        throw new Error(`Invalid providers: ${invalidProviders.join(", ")}. Valid providers are: ${validProviders.join(", ")}`);
                    }
                }
            }
        }
        ...
        beforeValidate: async (user, options) => {
            ...
            try {
                ...
                if (options.isNewRecord) {
                    ...
                    if (user.changed('providers')) changedFields.providers = user.providers;
                    
                    if (Object.keys(changedFields).length > 0) {
                        const validatedData = UserValidation.schemas.update.parse(changedFields);
                        Object.assign(user, validatedData);
                    }
                }
            } catch (error) {
